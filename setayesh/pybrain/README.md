...
## بازیابی برداری با embeddings (وقتی Ollama هست) 🧭

ستایش دو موتور بازیابی دارد و خودکار بهترینِ در دسترس را انتخاب می‌کند:
- **embeddings** (معناییِ واقعی) — وقتی Ollama + مدلِ `nomic-embed-text` نصب باشد. بردارها محلی کش می‌شوند.
- **TF-IDF** (محلی، بدون وابستگی) — بازگشت خودکار وقتی embeddings نیست.

تنظیم در `brain-settings.md`: `retrieval: auto|tfidf|embeddings` و `embeddings_model`.

## خلاصه‌سازی چند نوت 📝

ابزار `summarize` چند نوتِ مرتبط را می‌گیرد و یک خلاصهٔ تمیز می‌سازد (محلی، بدون مدل):
```bash
python setayesh/brain/server/main.py "خلاصه کن دربارهٔ بازیابی"
```

## گراف دانش بصری 🕸️

داشبورد وب (`--serve` → <http://localhost:8787>) گراف دانش والت را به‌صورت **SVG زنده**
نشان می‌دهد: گره‌ها = نوت‌ها (اندازه به‌نسبت تعداد اتصال)، یال‌ها = لینک‌های `[[...]]`.
نسخهٔ متنیِ گراف هم در `dashboard/DASHBOARD.md` هست.
...
```bash
bash setayesh/install.sh                 # نصب/به‌روزرسانی + راستی‌آزمایی با تست‌ها
...
bash setayesh/install.sh --dir ~/setayesh  # نصب در مسیر دلخواه
```
نسخهٔ فعلی: `python3 setayesh/brain/server/main.py --version`
