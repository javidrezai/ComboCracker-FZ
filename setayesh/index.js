...
// HTTPS policy. The phone's Web Bluetooth / Web Serial need a "secure context",
// which plain http on a LAN address is NOT — so for LAN/phone use (HOST bound to
// a network address) the self-signed secure link defaults ON (generated by
// selfsign.js, one-time browser warning). Pure localhost is ALREADY a secure
// context, so it stays on plain http — no needless cert warning, and the test
// harness (which binds 127.0.0.1) keeps talking http. An explicit AUTO_TLS in
// the config or environment always wins over this default.
function autoTlsEnabled() {
  const v = (cfg.AUTO_TLS != null && cfg.AUTO_TLS !== '') ? cfg.AUTO_TLS : process.env.SETAYESH_AUTO_TLS;
  if (v === '1' || v === 'true' || v === 'on') return true;
  if (v === '0' || v === 'false' || v === 'off') return false;
  return !(HOST === '127.0.0.1' || HOST === 'localhost' || HOST === '::1');
}
let AUTO_TLS_ON = autoTlsEnabled();
function loadTls() {
...
// zipEntries / officeToText / zipToText (office & ZIP → text) live in ./doctext,
// required at the top. xmlToText / htmlToText live in htmltext.js.
...
// TOOLS_SPEC (the AI tool JSON schemas) is pure data and lives in ./toolspec;
// toolsFor(ctx) below filters it per caller (permissions + feature flags).
...
// friendlyProviderError moved to providererror.js (pure, unit-tested).
...
// TUTORS (per-account hand-written personas) lives in ./personas.
...
app.get('/api/admin/activity', requireAuth, requireAdmin, (req, res) => {
  res.json({
    ...
    pending: knowledge.filter((k) => k.status === 'pending').length,
    // So the learning panel can explain, and offer to fix, the one thing that
    // actually stops research from working: having no usable engine.
    engines: Object.keys(PROVIDERS).filter(isConfigured),
    localEnabled: isConfigured('local'),
  });
});
...
// MAIL_PRESETS + mailConfigured + mailHost + imapFetch (the read-only IMAP
// client) live in ./mail; makeMail reads the live config via getCfg().
const _mail = require('./mail').makeMail({ getCfg: () => cfg });
const MAIL_PRESETS = _mail.MAIL_PRESETS;
function mailConfigured() { return _mail.mailConfigured(); }
function mailHost() { return _mail.mailHost(); }
function imapFetch(opts) { return _mail.imapFetch(opts); }
...
// Sync between the family's own computers is registered LATE (in sync.js),
// after board/memory/devices/knowledge and their savers exist — see below.
// ---------------- Notifying the owner ----------------
...
// Notifications + owner email + Telegram alert live in notify.js. notifyOwner
// is re-exposed as a hoisted wrapper so its existing call sites (and the
// `typeof notifyOwner === 'function'` guard) keep working unchanged. boardRoutes
// and pendingActions are defined later in this file, so they are passed lazily.
const _notify = require('./notify').register(app, {
  requireAuth, requireAdmin, isAdmin, cfg, MAIL_PRESETS,
  loadJsonFile, saveJsonFile, DATA_DIR, telegram,
  boardRoutes: { add: (m) => boardRoutes.add(m) },
  getPending: () => pendingActions,
});
function notifyOwner(opts) { return _notify.notifyOwner(opts); }
...
// in place, so in-place writes elsewhere (e.g. the sync merge) still work.
const memory = _mem.memory;
function memoryFor(u) { return _mem.memoryFor(u); }
function addMemory(u, e) { return _mem.addMemory(u, e); }
function memoryBlock(u) { return _mem.memoryBlock(u); }
function saveMemory() { return _mem.saveMemory(); }
...
// Device registration + prefs + admin list/revoke live in routes/devices.js.
// The trusted-device LOGIN flow and the devices state stay here; the module
// reaches `devices` via getDevices() because saveDevices() reassigns it on trim.
require('./routes/devices').register(app, {
  requireAuth, requireAdmin, getDevices: () => devices, saveDevices,
});

// EDITABLE_KEYS (the control-centre settings allowlist) is pure data and lives
// in ./configkeys.
...
app.post('/api/admin/secure-link', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    // Persist the choice. "Off" is stored as an explicit '0' (not '', which the
    // config writer would drop) so it survives — otherwise a LAN host, which
...
// ---------------- Sync between the family's own computers — sync.js ----------------
// Registered here, late, so board/memory/devices/knowledge and their savers all
// exist. knowledge and devices are reassigned in index.js, so they are reached
// through accessors (a captured reference would go stale); memory and
// boardRoutes are stable references passed directly. localLanIps is a hoisted
// function, so passing it here (before its textual definition) is fine.
require('./sync').register(app, {
  requireAuth, requireAdmin, loadJsonFile, saveJsonFile, DATA_DIR,
  fetchWithTimeout, localLanIps, boardRoutes, memory, saveMemory,
  saveDevices, saveKnowledge,
  getDevices: () => devices,
  getKnowledge: () => knowledge,
  setKnowledge: (v) => { knowledge = v; },
});
